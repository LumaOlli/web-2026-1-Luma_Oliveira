...
const menuItems: MenuItem[] = [
  ...
  { label: "Meus Agendamentos", icon: <CalendarListIcon />, to: "/aluno/minhas-monitorias" },
];
        ...
        <nav className="schedule-nav">
          {menuItems.map((item) => (
            <NavLink
              key={item.label}
              to={item.to}
              className={({ isActive }) => `schedule-menu-item${isActive ? " active" : ""}`}
            >
...
function SearchClockIcon() {
  return (
    <IconBase>
      ...
      <polyline points="11 8 11 11 12.5 12.5" />
    </IconBase>
  );
}

function PlusCalendarIcon() {
  return (
    <IconBase>
